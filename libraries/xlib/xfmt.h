...
struct xfmt_handler {
    const struct xfmt_handler_ops *    xf_ops;
    void *                             xf_priv;
};
typedef struct xfmt_handler xfmt_handler_t;

#define XFMT_HANDLER_INIT(ops,priv) { .xf_ops = (ops), .xf_priv = (priv) }
#define XFMT_HANDLER_PRIV(xf) ((xf)->xf_priv)

static inline int xfmt_handler_init(xfmt_handler_t *xf, const struct xfmt_handler_ops *ops, void *priv)
{
    xf->xf_ops = ops;
    xf->xf_priv = priv;
    return 0;
