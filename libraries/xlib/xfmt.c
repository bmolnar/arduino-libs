...
{
    if (xp->xf_ops->op_begin == NULL)
    {
        return -XERR_NOSYS;
    }
    return xp->xf_ops->op_begin(xp, pos);
}
static int
xfmt_handler_end(xfmt_handler_t *xp, const char *pos)
{
    if (xp->xf_ops->op_end == NULL)
    {
        return -XERR_NOSYS;
    }
    return xp->xf_ops->op_end(xp, pos);
}
static int
xfmt_handler_onval(xfmt_handler_t *xp, const char *pos, const char *end, const struct xfmt_spec *fmtspec, const xfmt_val_t *val)
{
    if (xp->xf_ops->op_onval == NULL)
    {
        return -XERR_NOSYS;
    }
    return xp->xf_ops->op_onval(xp, pos, end, fmtspec, val);
}
static int
xfmt_handler_onchars(xfmt_handler_t *xp, const char *pos, const char *end)
{
    if (xp->xf_ops->op_onchars == NULL)
    {
        return -XERR_NOSYS;
    }
    return xp->xf_ops->op_onchars(xp, pos, end);
}
static int
